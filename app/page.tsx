...
export default function Home() {
  ...
  return <main className="quiz-shell">
    ...
    {step >= questions.length && <section className="result-stage"><span className="eyebrow">SEU DIAGNÓSTICO</span><div className="result-number">0{questions.length}</div><h2>{result}</h2><p>A obra única pode ser apenas o sintoma. Quando método, modelo de negócio, capital, gestão e ecossistema não estão conectados, cada nova operação parece começar do zero.</p><div className="result-insight"><span>O PRÓXIMO PASSO</span><strong>Conhecer a lógica do Método 5/20</strong><small>Uma forma de organizar as peças de uma incorporação sem depender apenas do seu próprio capital.</small></div><a className="gold-button link-button" href="https://app.escoladeincorporadores.com.br/c/mbxta8z" target="_blank" rel="noreferrer">QUERO CONHECER A AULA <b>→</b></a><button className="restart" onClick={restart}>Refazer diagnóstico</button></section>}
