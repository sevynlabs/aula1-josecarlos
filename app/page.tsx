...
export default function Home() {
  ...
  async function next() {
    ...
    if (step === questions.length - 1) {
      const finalDiagnosis = finalAnswers.filter((a) => a === 0 || a === 1).length >= 3 ? "Você está no ciclo da obra única" : "Você já percebeu que precisa de um próximo modelo";
      await fetch("https://script.google.com/macros/s/AKfycbw-H3AN7PfbWEbrucjjcniayAkWpd_FOZPJIkQblvzTQZMpy1346BCwoiwIFXFCmUlz7g/exec", { method: "POST", mode: "no-cors", headers: { "Content-Type": "text/plain;charset=utf-8" }, body: JSON.stringify({ ...lead, diagnostico: finalDiagnosis, respostas: finalAnswers.map((answer, index) => `${questions[index].title}: ${questions[index].options[answer]}`), concluido: "Sim" }) });
    }
